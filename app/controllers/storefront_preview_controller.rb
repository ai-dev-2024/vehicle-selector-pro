...
class StorefrontPreviewController < ActionController::Base
  ...
  def product
    ...
    @fitments = shop&.vehicle_product_fitments&.where(product_id: @product[:product_id])&.map do |f|
      f.universal_fit? ? 'Universal — fits all vehicles' : f.vehicle&.display_name
    end&.compact&.uniq || []
  end
  ...
  def render_demo_404
    render html: '<p style="padding:48px;text-align:center;font-family:sans-serif">Product not found. <a href="/demo">Back to shop</a></p>'.html_safe,

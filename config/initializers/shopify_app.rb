ShopifyApp.configure do |config|
  ...
  config.old_secret = ""
  # Scopes for managing vehicle fitment metafields
  config.scope = "read_products,write_products,read_product_listings,read_customers,write_customers"
  ...
  config.api_key = ENV.fetch("SHOPIFY_API_KEY") do
    Rails.application.credentials.dig(:shopify, :api_key) || raise("SHOPIFY_API_KEY missing")
  end
  config.secret = ENV.fetch("SHOPIFY_API_SECRET") do
    Rails.application.credentials.dig(:shopify, :api_secret) || raise("SHOPIFY_API_SECRET missing")
  end
end

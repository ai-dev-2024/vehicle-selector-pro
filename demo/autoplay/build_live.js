...
const FRAMES = path.join(ROOT, 'frames_live');
const AUDIO = path.join(ROOT, 'audio_live');
const TEMP = path.join(ROOT, 'temp_live');
const OUTPUT = path.join(ROOT, '..', 'Vehicle_Selector_Pro_Demo.mp4');

const manifest = JSON.parse(fs.readFileSync(path.join(ROOT, 'manifest_live.json'), 'utf8'));
...
for (const scene of manifest) {
  ...
  if (!fs.existsSync(img)) {
    console.log(`SKIP ${scene.name} - no image`);
    continue;
  }
  if (!fs.existsSync(wav)) {
    console.log(`SKIP ${scene.name} - no audio`);
    continue;
  }
  ...
  try {
    execSync(`ffmpeg -y -loop 1 -i "${img}" -i "${wav}" -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" -c:v libx264 -preset ultrafast -tune stillimage -pix_fmt yuv420p -af "aresample=44100" -c:a aac -b:a 128k -strict -2 -t ${totalDur} -shortest "${seg}"`, { stdio: 'pipe' });
